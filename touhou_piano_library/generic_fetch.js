...
async function create_person_link(name) {
    ...
    var arrangers = await get_json("arrangers.json");
...
async function create_theme_link(id) {
    ...
    var themes = await get_json("touhou_original_themes.json");
    if (id in themes) {
...
async function get_arrangement_title(data) {
    ...
    if (data.title === null) {
        var theme_data = await get_json("touhou_original_themes.json");
